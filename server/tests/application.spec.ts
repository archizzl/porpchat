...
import { Answer, Question, Tag, Comment, Message, Thread, Account } from '../types';
...
// Messages
const msg1: Message = {
  _id: new ObjectId('65e9b58910afe6e94fc6e6e1'),
  sender: 'user1',
  messageDateTime: new Date('2023-11-15T08:30:00'),
  content: 'Hello! Is the project ready?',
  views: ['user2', 'user3'],
  likes: ['user2'],
};

const msg2: Message = {
  _id: new ObjectId('65e9b58910afe6e94fc6e6e2'),
  sender: 'user2',
  messageDateTime: new Date('2023-11-15T08:45:00'),
  content: 'Not yet, I need some more time.',
  views: ['user1', 'user3'],
  likes: ['user1'],
};

const msg3: Message = {
  _id: new ObjectId('65e9b58910afe6e94fc6e6e3'),
  sender: 'user3',
  messageDateTime: new Date('2023-11-16T10:00:00'),
  content: 'Hey, let me know if I can help.',
  views: ['user1', 'user2'],
  likes: ['user1', 'user2'],
};

const msg4: Message = {
  _id: new ObjectId('65e9b58910afe6e94fc6e6e4'),
  sender: 'user1',
  messageDateTime: new Date('2023-11-16T11:00:00'),
  content: 'Thanks, I’ll reach out if needed.',
  views: ['user2'],
  likes: ['user3'],
};
...
const THREADS: Thread[] = [
  {
    _id: new ObjectId('65e9b58910afe6e94fc6e6f1'),
    messages: [msg1, msg2],
    accounts: ['user1', 'user2'],
    threadUpdatedDateTime: new Date('2023-11-15T08:45:00'),
  },
  {
    _id: new ObjectId('65e9b58910afe6e94fc6e6f2'),
    messages: [msg3, msg4],
    accounts: ['user1', 'user3'],
    threadUpdatedDateTime: new Date('2023-11-16T11:00:00'),
  },
  {
    _id: new ObjectId('65e9b58910afe6e94fc6e6f3'),
    messages: [],
    accounts: ['user2', 'user3'],
    threadUpdatedDateTime: new Date('2023-11-14T09:00:00'),
  },
  {
    _id: new ObjectId('65e9b58910afe6e94fc6e6f4'),
    messages: [msg1, msg3],
    accounts: ['user1', 'user2', 'user3'],
    threadUpdatedDateTime: new Date('2023-11-16T10:00:00'),
  },
];

// Accounts
const account1: Account = {
  _id: new ObjectId('65e9b58910afe6e94fc6e6a1'),
  username: 'user1',
  email: 'user1@example.com',
  password: 'hashedpassword1',
  createdAt: new Date('2023-11-01T08:00:00'),
  questions: [],
  answers: [ans1, ans2],
...
const account2: Account = {
  _id: new ObjectId('65e9b58910afe6e94fc6e6a2'),
  username: 'user2',
  email: 'user2@example.com',
  password: 'hashedpassword2',
  createdAt: new Date('2023-11-02T09:00:00'),
...
const account3: Account = {
  _id: new ObjectId('65e9b58910afe6e94fc6e6a3'),
  username: 'user3',
...
describe('application module', () => {
  ...
  describe('Message model', () => {
    describe('saveMessage', () => {
      test('saveMessage should return the saved message', async () => {
        ...
        expect(result.sender).toEqual(msg1.sender);
        expect(result.content).toEqual(msg1.content);
      });
    });
    describe('addMessageToThread', () => {
      test('addMessageToThread should return the updated thread', async () => {
        const thread = { ...THREADS[0], messages: [msg1, msg2, msg3] };
        mockingoose(ThreadModel).toReturn(thread, 'findOneAndUpdate');

        const result = (await addMessageToThread(thread._id?.toString() as string, msg3)) as Thread;
        expect(result.messages.length).toEqual(3);
        expect(result.messages).toContain(msg3._id);
      });
      test('addMessageToThread should return an object with error if findOneAndUpdate throws an error', async () => {
        const thread = THREADS[0];
        mockingoose(ThreadModel).toReturn(
          new Error('Error from findOneAndUpdate'),
          'findOneAndUpdate',
        );
        const result = await addMessageToThread(thread._id?.toString() as string, msg3);
        expect(result).toEqual({ error: 'Error when adding message' });
      });
      test('addMessageToThread should return an object with error if findOneAndUpdate returns null', async () => {
        const thread = THREADS[0];
        mockingoose(ThreadModel).toReturn(null, 'findOneAndUpdate');
        const result = await addMessageToThread(thread._id?.toString() as string, msg3);
        expect(result).toEqual({ error: 'Error when adding message' });
      });
      test('addMessageToThread should throw an error if a required field is missing in the message', async () => {
        ...
        const tid = 'validThreadId';
        try {
          await addMessageToThread(tid, invalidMessage as Message);
        } catch (err: unknown) {
    ...
    describe('addLikeToMessage', () => {
      test('should like a message', async () => {
        const mockMessage = {
          _id: 'someMessageId',
          likes: [],
        };
        ...
        mockingoose(MessageModel).toReturn(
          { ...mockMessage, likes: ['testUser'] },
          'findOneAndUpdate',
        );

        const result = await addLikeToMessage('someMessageId', 'testUser');
      ...
      test('should cancel the like if already liked', async () => {
        const mockMessage = {
          _id: 'someMessageId',
          likes: ['testUser'],
        };

        mockingoose(MessageModel).toReturn({ ...mockMessage, likes: [] }, 'findOneAndUpdate');
      ...
      test('should return an error if the message is not found', async () => {
        mockingoose(MessageModel).toReturn(null, 'findById');

        const result = await addLikeToMessage('nonExistentId', 'testUser');

        expect(result).toEqual({ error: 'Message not found!' });
      });

      test('should return an error when there is an issue with liking a message', async () => {
        mockingoose(MessageModel).toReturn(new Error('Database error'), 'findOneAndUpdate');

        const result = await addLikeToMessage('someMessageId', 'testUser');

        expect(result).toEqual({ error: 'Error when adding like to message' });
      });
    });
  });
  ...
  describe('Thread model', () => {
    describe('saveThread', () => {
      test('saveThread should return the saved thread', async () => {
        const result = (await saveThread(THREADS[0])) as Thread;
        expect(result._id).toBeDefined();
        expect(result.accounts).toEqual(['user1', 'user2']);
      });

      // test('getThreadsFromUser should return all threads a user is involved with', async () => {
      //   mockingoose(AccountModel).toReturn(account1, 'findOne');
      //   // expect(account1).toEqual(5)

      //   const result = (await getThreadsFromUser('user1')) as Thread[];
  ...
  describe('Account model', () => {
    ...
    //   test('should return an error if account creation fails', async () => {
    ...
    //       threads: [],
    //     };
    //     mockingoose(AccountModel).toReturn(new Error('Error creating account'), 'save');
    //     const result = await saveAccount(accountData);
    //     expect(result).toEqual({ error: 'Error creating account' });
    //   });
    // });
    /*
    describe('authenticateAccount', () => {
      test('should authenticate a valid account and return a JWT token', async () => {
        const accountData = {
          _id: new ObjectId(),
          username: 'testUser',
          email: 'test@example.com',
        ...
        mockingoose(AccountModel).toReturn(accountData, 'findOne');
        const result = await authenticateAccount(accountData.email, 'plainpassword');
        expect(result).toHaveProperty('token');
        expect(typeof result.token).toBe('string');
        // expect(jwt.verify(result.token!, process.env.JWT_SECRET || 'supersecretkey')).toBeTruthy();
      });
      test('should return an error if the account is not found', async () => {
        mockingoose(AccountModel).toReturn(null, 'findOne');
        const result = await authenticateAccount('nonexistent@example.com', 'plainpassword');
      ...
      test('should return an error if the password is incorrect', async () => {
        ...
        mockingoose(AccountModel).toReturn(accountData, 'findOne');
        const result = await authenticateAccount(accountData.email, 'wrongpassword');
        expect(result).toEqual({ error: 'Invalid credentials' });
      });
      test('should return an error if an exception occurs during authentication', async () => {
        mockingoose(AccountModel).toReturn(new Error('Database error'), 'findOne');
        const result = await authenticateAccount('test@example.com', 'plainpassword');
        expect(result).toEqual({ error: 'Error during authentication' });
      });
    });
    */
  });
